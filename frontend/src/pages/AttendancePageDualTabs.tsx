...
function AttendancePageDualTabs() {
  const { user, token, logout } = useAuth();
  
  // Tab State
  const [activeMode, setActiveMode] = useState<AttendanceMode>('face');
  
  // Camera states (Face Recognition)
  ...
  useEffect(() => {
    ...
    // Auto-start camera jika mode Face dan tidak registering
    if (activeMode === 'face' && !isRegistering) {
      setTimeout(() => {
        startCamera();
      }, 500);
    }
  ...
  // Handle mode switch
  const handleModeSwitch = (mode: AttendanceMode) => {
    if (mode === activeMode) return;
    
    // Stop current mode
    if (activeMode === 'face' && isCameraOn) {
      stopCamera();
    }
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      if (!videoRef.current) {
        showMessage('Error: Video element tidak tersedia.', 'error');
  ...
  const checkFaceQualityRealtime = async () => {
    if (!isCameraOn || isCheckingQuality || activeMode !== 'face') return;
  ...
  const recognizeFace = async () => {
    if (!qualityResult?.is_acceptable) {
      showMessage('⚠️ Kualitas wajah belum memenuhi standar.', 'error');
      return;
    }
    ...
    try {
      ...
      if (response.data.status === 'success' && response.data.detected) {
        ...
        if (!response.data.already_marked) {
          showMessage(`🎉 Absensi berhasil! Selamat datang, ${response.data.name}`, 'success');
          loadAttendanceRecords();
          checkAttendanceStatus();
  ...
  const handleQRScanSuccess = async (qrData: string) => {
    ...
    try {
      const response = await axios.post(
        `${BACKEND_URL}/api/attendance/qr`,
        { qr_data: qrData },
        { headers: { 'Authorization': `Bearer ${token}` } }
      );

      if (response.data.status === 'success') {
        showMessage(`🎉 Absensi berhasil! ${response.data.message}`, 'success');
        loadAttendanceRecords();
        checkAttendanceStatus();
        setQrScanResult('');
      } else {
        showMessage(response.data.message || 'QR Code tidak valid', 'error');
      }
    } catch (error: any) {
      if (error.response?.status === 401) {
        showMessage('Sesi berakhir. Silakan login kembali.', 'error');
        ...
        showMessage(error.response?.data?.message || 'Gagal memproses QR Code', 'error');
      }
    } finally {
      setIsProcessing(false);
    }
  ...
  // Multi-Photo Capture (untuk registrasi)
  ...
  const capturePhotoAtIndex = (photoIndex: number) => {
    ...
    if (photoIndex + 1 < TOTAL_PHOTOS) {
      ...
      setIsCapturingMulti(false);
      setCountdown(0);
      showMessage(`✅ ${TOTAL_PHOTOS} foto berhasil diambil!`, 'success');
    }
  };
  ...
  const handleRegisterMultiPhoto = async () => {
    if (capturedPhotos.length !== TOTAL_PHOTOS) {
      showMessage(`Diperlukan ${TOTAL_PHOTOS} foto`, 'error');
      return;
    }
      ...
      <div className="flex-1 ml-64 overflow-y-auto">
        <div className="max-w-7xl mx-auto p-6 sm:p-8 space-y-6">
            ...
            <div className="flex justify-end">
              <button
                onClick={() => {
                  ...
                  if (!isRegistering && !isCameraOn && activeMode === 'face') {
                    startCamera();
                  }
                }}
                disabled={activeMode === 'qr'}
                className="px-6 py-3 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-xl font-semibold hover:from-red-700 hover:to-red-800 transition-all shadow-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isRegistering ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Left Panel: Attendance Methods */}
            <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
              {/* Tab Header */}
              <div className="flex items-center gap-2 mb-6 bg-gray-100 p-2 rounded-xl">
                <button
              ...
              {activeMode === 'face' ? (
                            ...
                            {!isCapturingMulti && capturedPhotos.length === 0 ? (
                              <>
                                <div className="flex items-start gap-2 text-red-800 text-sm">
                                  ...
                                  <p>Sistem akan mengambil <strong>3 foto otomatis</strong> dengan jeda 3 detik</p>
                                </div>
                              ...
                              <>
                                <div className="flex items-start gap-2 text-green-800 text-sm bg-green-100 p-3 rounded-lg">
                                  ...
                                  <p><strong>3 foto berhasil diambil!</strong> Review dan submit.</p>
                                </div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <ul className="text-xs text-blue-800 space-y-1">
                            ...
                            <li className="flex items-center gap-2">
                              <CheckCircle2 className="w-3 h-3 text-green-500" />
                              <span>Lepas masker atau kacamata hitam</span>
                            </li>
                            <li className="flex items-center gap-2">
                              <CheckCircle2 className="w-3 h-3 text-green-500" />
                              <span>Jaga jarak 40-60 cm dari kamera</span>
                            </li>
                          </ul>
                        </div>
                      </div>
                ...
                <>
                  ...
                  <QRScanner
                    ...
                    onScanError={(error) => showMessage(error, 'error')}
                        ...
                        <div className="text-right">
                          ...
                          {record.check_in_type && (
                            <span className="text-xs text-gray-500 mt-1 block">
                              via {record.check_in_type === 'qr' ? 'QR Code' : 'Face Recognition'}
                            </span>
                          )}
                        </div>
                      </div>
...
export default AttendancePageDualTabs;
